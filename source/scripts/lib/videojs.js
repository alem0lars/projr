...
// Set CDN Version of swf
  if (CDN_VERSION != "GENERATED_CDN_VSN") {
    _V_.options.flash.swf =
        "http://vjs.zencdn.net/" + CDN_VERSION + "/video-js.swf"
  }
  _V_.merge = function (obj1, obj2, safe) {
    ...
    for (var attrname in obj2) {
      if (obj2.hasOwnProperty(attrname) && (!safe
          || !obj1.hasOwnProperty(attrname))) {
  ...
  _V_.extend({
    ...
    addClass: function (element, classToAdd) {
      if ((" " + element.className + " ").indexOf(" " + classToAdd + " ")
          == -1) {
        element.className =
            element.className === "" ? classToAdd : element.className + " "
                + classToAdd;
      }
    },
    ...
    getRelativePosition: function (x, relativeElement) {
      return Math.max(0, Math.min(1,
          (x - _V_.findPosX(relativeElement)) / relativeElement.offsetWidth));
    },
    ...
    get: function (url, onSuccess, onError) {
      ...
      var local = (url.indexOf("file:") == 0
          || (window.location.href.indexOf("file:") == 0 && url.indexOf("http:")
          == -1));
...
// make it safe to use console.log always
  (function (b) {
    ...
    for (var d = "assert,count,debug,dir,dirxml,error,exception,group,groupCollapsed,groupEnd,info,log,timeStamp,profile,profileEnd,time,timeEnd,trace,warn".split(","), a;
         a = d.pop();) {
      b[a] = b[a] || c
    }
  })((function () {
  ...
  _V_.Button = _V_.Control.extend({
    ...
    createElement: function (type, attrs) {
      ...
      attrs = _V_.merge({
        ...
        innerHTML: '<div><span class="vjs-control-text">' + (this.buttonText
            || "Need Text") + '</span></div>',
        role: "button",
        tabIndex: 0
      }, attrs);
...
// Load Progress Bar
  _V_.LoadProgressBar = _V_.Component.extend({
    ...
    update: function () {
      if (this.el.style) {
        this.el.style.width =
            _V_.round(this.player.bufferedPercent() * 100, 2) + "%";
      }
    }
  ...
  _V_.extend({
    ...
    fixEvent: function (event) {
      ...
      if (!event.relatedTarget && event.fromElement) {
        event.relatedTarget =
            event.fromElement === event.target ? event.toElement
                : event.fromElement;
      }
      ...
      if (event.pageX == null && event.clientX != null) {
        ...
        event.pageX =
            event.clientX + (doc && doc.scrollLeft || body && body.scrollLeft
                || 0) - (doc && doc.clientLeft || body && body.clientLeft || 0);
        event.pageY =
            event.clientY + (doc && doc.scrollTop || body && body.scrollTop
      ...
      if (!event.which && event.button !== undefined) {
        event.which =
            (event.button & 1 ? 1 : ( event.button & 2 ? 3 : ( event.button & 4
                ? 2 : 0 ) ));
      }
...
// https://github.com/douglascrockford/JSON-js/blob/master/json2.js
  ...
  _V_.Player = _V_.Component.extend({
    ...
    loadTech: function (techName, source) {
      ...
      var techOptions = _V_.merge({ source: source, parentEl: this.el },
      ...
      if (this.tech.isReady) {
        ...
        try {
          ...
          if (this.tech[method] === undefined) {
            _V_.log("Video.js: " + method + " method not defined for "
            ...
            if (e.name == "TypeError") {
              _V_.log("Video.js: " + method + " unavailable on " + this.techName
        ...
        _V_.addEvent(document, requestFullScreen.eventName,
            this.proxy(function () {
              ...
              if (this.isFullScreen == false) {
                _V_.removeEvent(document, requestFullScreen.eventName,
                    arguments.callee);
              }
    ...
    enterFullWindow: function () {
      ...
      _V_.addEvent(document, "keydown",
          _V_.proxy(this, this.fullWindowOnEscKey));
...
// Create placeholder methods for each that warn when a method isn't supported by the current playback technology
  ...
  _V_.html5 = _V_.PlaybackTech.extend({
    ...
    removeTriggers: function () {
      _V_.each.call(this, _V_.html5.events, function (type) {
        _V_.removeEvent(this.el, type,
            _V_.proxy(this.player, this.eventHandler));
      });
    },
...
// List of all HTML5 events (various uses).
  ...
  _V_.html5.prototype.support = {
    ...
    fullscreen: (typeof _V_.testVid.webkitEnterFullScreen !== undefined)
        ? (!_V_.ua.match("Chrome") && !_V_.ua.match("Mac OS X 10.5") ? true
        : false) : false,
  ...
  if (_V_.isAndroid()) {
    ...
    if (_V_.androidVersion() < 3) {
      document.createElement("video").constructor.prototype.canPlayType =
          function (type) {
            return (type && type.toLowerCase().indexOf("video/mp4") != -1)
                ? "maybe" : "";
          };
    }
  }
  ...
  _V_.flash = _V_.PlaybackTech.extend({
    ...
    init: function (player, options) {
      ...
      // Create a temporary element to be replaced by swf object
          placeHolder = this.el =
              _V_.createElement("div", { id: parentEl.id + "_temp_flash" }),
      ...
      if (options.iFrameMode == true && !_V_.isFF) {
        ...
        _V_.addEvent(iFrm, "load", _V_.proxy(this, function () {
          ...
          iDoc.write(_V_.flash.getEmbedCode(options.swf, flashVars, params,
              attributes));
          ...
          iWin.events =
              _V_.proxy(this.player, function (swfID, eventName, other) {
                var player = this;
        ...
        _V_.flash.embed(options.swf, placeHolder, flashVars, params,
            attributes);
      }
    },
...
// Flash Version Check
  _V_.flash.version = function () {
    ...
    try {
      version =
          new ActiveXObject('ShockwaveFlash.ShockwaveFlash').GetVariable('$version').replace(/\D+/g,
              ',').match(/^,?(.+),?$/)[1];
  ...
  _V_.Track = _V_.Component.extend({
    ...
    init: function (player, options) {
      ...
      _V_.merge(this, {
        ...
        id: options.id || ("vjs_" + options.kind + "_" + options.language + "_"
            + _V_.guid++),
...
// Global container for both subtitle and captions text. Simple div container.
  ...
  _V_.TextTrackMenuItem = _V_.MenuItem.extend({
    ...
    init: function (player, options) {
      ...
      this.player.addEvent(track.kind + "trackchange",
          _V_.proxy(this, this.update));
    },
  ...
  _V_.TextTrackButton = _V_.Button.extend({
    ...
    onFocus: function () {
      ...
      _V_.one(this.menu.el.childNodes[this.menu.el.childNodes.length - 1],
